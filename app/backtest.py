...
def evaluate_trend_breakout(oh, ema50_4h, btc_regime):
    """Strategy 2 decision. Same return shape as evaluate(): (is_long, entry, sl,
    tp1, tp2, conviction) for the last bar, or None if no setup."""
    prices = [x[4] for x in oh]
    highs = [x[2] for x in oh]
    lows = [x[3] for x in oh]
    volumes = [x[5] for x in oh]
    if len(prices) < max(C.MIN_CANDLES_FOR_SIGNAL, DONCHIAN_PERIOD + 30):
        return None
    price = prices[-1]
    ...
    # Donchian breakout over the PRIOR `DONCHIAN_PERIOD` bars (exclude current).
    upper = max(highs[-DONCHIAN_PERIOD - 1:-1])
    lower = min(lows[-DONCHIAN_PERIOD - 1:-1])
    broke_up = price > upper
    broke_dn = price < lower
    if not (broke_up or broke_dn):
        return None
    is_long = broke_up

    # Macro trend filter — trade breakouts only WITH the EMA200 trend.
    if is_long and price < ema200:
        return None
    if (not is_long) and price > ema200:
        return None

    # ADX — require a real trend (filters chop / fakeout breakouts).
    adx = calculate_adx(oh, ADX_PERIOD)
    if adx is None or adx < ADX_MIN:
        return None

    # Volume confirmation — weak-volume breakouts are traps.
    if not check_volume_gate(volumes, C.VOLUME_GATE_MULTIPLIER):
        return None

    # BTC regime — shared "don't fight Bitcoin" risk control.
    if C.ENABLE_BTC_REGIME_FILTER:
        if is_long and btc_regime == "bear":
            return None
        if (not is_long) and btc_regime == "bull":
            return None
    if not getattr(C, "ENABLE_SHORTS", True) and not is_long:
        return None

    # Entry at the broken level (breakout-retest), ATR stop, genuine 1R/2R targets
    # — identical management to Strategy 1 so only the entry signal is compared.
    atr = bot.calculate_atr(oh)
    if not atr:
        return None
    entry = upper if is_long else lower
    sl_dist = atr * C.ATR_SL_MULTIPLIER
    if is_long:
        sl = entry - sl_dist
        if (entry - sl) / entry > C.MAX_SL_PCT:
            sl = entry * (1 - C.MAX_SL_PCT)
    else:
        sl = entry + sl_dist
        if (sl - entry) / entry > C.MAX_SL_PCT:
            sl = entry * (1 + C.MAX_SL_PCT)
    risk = abs(entry - sl)
    if risk <= 0:
        return None
    if is_long:
        tp1, tp2 = entry + risk * C.ATR_TP1_MULTIPLIER, entry + risk * C.ATR_TP_MULTIPLIER
    else:
        tp1, tp2 = entry - risk * C.ATR_TP1_MULTIPLIER, entry - risk * C.ATR_TP_MULTIPLIER

    # "lights" is a Strategy-1 concept; report a flat conviction so the tier table
    # still renders something sensible for this single-tier strategy.
    return (is_long, entry, sl, tp1, tp2, 5)


# ── Strategy 2 (replacement): Volatility-Squeeze Breakout (TTM Squeeze) ──────
# The old S2 breakout died on FALSE breakouts in chop. The evidence-based fix
# (John Carter's TTM Squeeze): only trade a breakout that emerges from a volatility
# SQUEEZE — when the Bollinger Bands have contracted INSIDE the Keltner Channels
# (coiling / energy building), and then RELEASE (BB expand back outside the KC).
# Breakouts born from compression are the high-probability ones; the rest is noise.
# Direction comes from the squeeze-momentum oscillator (linreg of price vs the
# range midpoint). Everything else mirrors the old S2 (EMA200 + volume + BTC regime,
# retest entry, ATR stop, 1R/2R) so only the entry-QUALITY filter changes.
SQZ_LENGTH = 20
SQZ_BB_STD = 2.0
SQZ_KC_ATR_MULT = 1.5     # Keltner width; BB inside KC ⇒ squeeze ON
SQZ_LOOKBACK = 6          # a squeeze must have been ON within this many prior bars
SQZ_ATR_BUFFER = 0.5      # the release breakout must clear the channel by 0.5×ATR


def evaluate_squeeze_breakout(oh, ema50_4h, btc_regime):
    """Strategy 2 candidate — volatility-compression (TTM Squeeze) breakout.
    Same return shape as evaluate(), or None."""
    prices = [x[4] for x in oh]
    highs = [x[2] for x in oh]
    lows = [x[3] for x in oh]
    volumes = [x[5] for x in oh]
    ...
    basis = s.rolling(L).mean()
    dev = SQZ_BB_STD * s.rolling(L).std()
    up_bb, lo_bb = basis + dev, basis - dev
    tr = pd.concat([hi - lo, (hi - s.shift()).abs(), (lo - s.shift()).abs()], axis=1).max(axis=1)
    rng = tr.rolling(L).mean()
    up_kc, lo_kc = basis + rng * SQZ_KC_ATR_MULT, basis - rng * SQZ_KC_ATR_MULT
    sqz_on = (lo_bb > lo_kc) & (up_bb < up_kc)
    if bool(sqz_on.iloc[-1]):
        return None                              # still coiling — wait for the release
    prior = sqz_on.iloc[-(SQZ_LOOKBACK + 1):-1]
    if prior.isna().all() or not bool(prior.any()):
        return None                              # no recent compression → not a squeeze break

    # Squeeze-momentum direction: linreg of (close − range-midpoint) over the window.
    hh, ll = hi.rolling(L).max(), lo.rolling(L).min()
    avg2 = ((hh + ll) / 2 + basis) / 2
    y = (s - avg2).iloc[-L:].to_numpy()
    if np.any(np.isnan(y)):
        return None
    x = np.arange(L)
    slope, intercept = np.polyfit(x, y, 1)
    val = slope * (L - 1) + intercept            # linreg value at the latest bar
    is_long = val > 0

    # Release breakout must clear the prior Donchian channel by an ATR buffer.
    upper = max(highs[-DONCHIAN_PERIOD - 1:-1])
    lower = min(lows[-DONCHIAN_PERIOD - 1:-1])
    buf = SQZ_ATR_BUFFER * atr
    if is_long and not (price > upper + buf):
        return None
    if (not is_long) and not (price < lower - buf):
        return None

    # Shared filters (identical to the old S2): macro trend, volume, BTC regime.
    if is_long and price < ema200:
        return None
    if (not is_long) and price > ema200:
        return None
    ...
    # Retest entry at the broken level, ATR stop, 1R/2R — same management as old S2.
    entry = upper if is_long else lower
    sl_dist = atr * C.ATR_SL_MULTIPLIER
    if is_long:
        sl = entry - sl_dist
...
# ── trailing-exit simulator (Strategy 3) — let winners run ───────────────────
# Trend-following dies under a fixed 2R cap (Strategy 2's weakness). This manages
# the trade with a Chandelier-style ATR trailing stop instead: the stop ratchets
# behind the best price reached and never loosens, so a strong trend can run far
# beyond 2R while a reversal still books whatever was locked in. The initial stop
# is the same ATR×1.5 structure stop; once price advances it trails at
# CHANDELIER_ATR_MULT × ATR behind the high-water mark.
CHANDELIER_ATR_MULT = 3.0   # trailing distance behind the run's peak/trough
MAX_HOLD_BARS_TRAIL = 400   # trends can last — give the runner more room than the bracket


def simulate_trade_trailing(oh15, i, is_long, entry, sl, tp1, tp2):
    """Resting-limit fill, then a ratcheting ATR trailing stop (no fixed target).
    Returns (pnl_pct, close_bar) or None if the entry never filled. tp1/tp2 are
    accepted for a uniform dispatch signature but intentionally unused here."""
    n = len(oh15)
    # 1) resting entry fill (same model as simulate_trade, no target-hit cancel
    #    since a trailing trade has no fixed target to be pre-empted by).
    fill = None
    for j in range(i + 1, min(i + 1 + MAX_WAIT_BARS, n)):
        hi, lo = oh15[j][2], oh15[j][3]
        if (is_long and lo <= entry) or (not is_long and hi >= entry):
    ...
    # ATR as of the signal bar sets the trailing distance.
    atr = bot.calculate_atr(oh15[:i + 1])
    if not atr or atr <= 0:
        atr = abs(entry - sl) / max(C.ATR_SL_MULTIPLIER, 0.5)  # fallback from the structure stop
    trail = atr * CHANDELIER_ATR_MULT

    stop = sl
    peak = entry          # high-water mark for longs
    trough = entry        # low-water mark for shorts
    end = min(fill + MAX_HOLD_BARS_TRAIL, n)
    for k in range(fill, end):
        hi, lo = oh15[k][2], oh15[k][3]
        if is_long:
            if lo <= stop:                               # stop hit (conservative: check before raising)
                return (stop - entry) / entry * 100, k
            peak = max(peak, hi)
            stop = max(stop, peak - trail)               # ratchet up only
        else:
            if hi >= stop:
                return (entry - stop) / entry * 100, k
            trough = min(trough, lo)
            stop = min(stop, trough + trail)             # ratchet down only
    # not stopped within the hold cap → mark to last close
    last = oh15[end - 1][4]
...
STRATEGIES = {
    ...
    # NOTE: the registry KEY stays "trend_breakout" (it's persisted in the DB
    # `strategy` column, the LIVE_STRATEGY env var, and the web dropdown). Only the
    # decision engine changed: S2 is now mean reversion, not a breakout. S3
    # ("trend_trailing") still uses the original evaluate_trend_breakout below.
    "trend_breakout": {
        "name": "Strategy 2 — Volatility-Squeeze Breakout",
        "desc": "Only trades a breakout that emerges from a volatility SQUEEZE (Bollinger Bands compressed inside the Keltner Channels, then releasing) — John Carter's TTM Squeeze. The compression filter kills the false breakouts that made the old raw Donchian breakout the worst strategy in the book.",
        "fn": evaluate_squeeze_breakout,
        "sim": simulate_trade,
    },
    "trend_trailing": {
        "name": "Strategy 3 — Trend Breakout + Trailing Stop",
        "desc": "Same breakout entry as Strategy 2 but managed with a Chandelier ATR trailing stop, so winners run far beyond 2R instead of being capped.",
        "fn": evaluate_trend_breakout,
        "sim": simulate_trade_trailing,
    },
    # Strategy 4 (adaptive_trend) and Strategy 5 (ict_smc) were REMOVED 2026-06-22:
    # both lost the cross-strategy bake-off badly — net-negative on clean crypto in
    # every window (S4 −0.30 to −0.34R, S5 −0.24 to −0.32R), while S1 was the only
    # net-positive engine. Their decision engines + tests were fully deleted
    # 2026-06-23. Re-adding either means restoring its evaluate_* function and one
    # entry here.
}
