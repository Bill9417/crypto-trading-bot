...
def _news_alerts(items: list, state: dict, now: float) -> list:
    ...
    for it in items:
        ...
        out.append(f"{cat.split()[0]} BIG EVENT · {cat}\n"
                   f"{it.get('title')}\n"
...
def _calendar_alerts(events: list, state: dict, now: float) -> list:
    ...
    for ev in events:
        ...
        extra = " · ".join(x for x in (
            f"forecast {ev['forecast']}" if ev.get("forecast") else "",
            f"previous {ev['previous']}" if ev.get("previous") else "") if x)
        out.append(f"🗓 MACRO in {mins} min · {ev.get('title')} (US)\n"
                   + (extra + "\n" if extra else "")
                   + "High-impact print — mind leveraged positions.")
    return out
...
def _shock_alerts(client, state: dict, now: float) -> list:
    ...
    for sym in SHOCK_SYMBOLS:
        ...
        if abs(chg_15m) >= SHOCK_15M_PCT or abs(chg_1h) >= SHOCK_1H_PCT:
            ...
            out.append(f"💥 MARKET SHOCK · {arrow} {base} "
                       f"{chg_15m:+.1f}% in 15m · {chg_1h:+.1f}% in 1h "
                       f"@ {closes[-1]:,.6g}\n"
                       f"Sudden move — check news / liquidation cascade.")
    return out
