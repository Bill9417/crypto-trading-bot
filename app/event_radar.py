...
def _news_alerts(items: list, state: dict, now: float) -> list:
    ...
    for it in items:
        ...
        out.append(f"{cat.split()[0]} 重大事件 BIG EVENT · {cat}\n"
                   f"{it.get('title')}\n"
...
def _calendar_alerts(events: list, state: dict, now: float) -> list:
    ...
    for ev in events:
        ...
        out.append(f"🗓 總經數據 MACRO · {mins} 分鐘後 · {ev.get('title')} (美國)\n"
                   + (extra + "\n" if extra else "")
                   + "高影響數據公布 — 有槓桿倉位的注意波動。")
    return out
...
def _shock_alerts(client, state: dict, now: float) -> list:
    ...
    for sym in SHOCK_SYMBOLS:
        ...
        if abs(chg_15m) >= SHOCK_15M_PCT or abs(chg_1h) >= SHOCK_1H_PCT:
            ...
            out.append(f"💥 行情劇變 MARKET SHOCK · {arrow} {base} "
                       f"15分鐘 {chg_15m:+.1f}% · 1小時 {chg_1h:+.1f}% "
                       f"@ {closes[-1]:,.6g}\n"
                       f"突發劇烈波動 — 留意新聞面 / 清算連鎖，槓桿倉位先檢查。")
    return out
