...
/resume (or /halt to trip it manually). Exits are NEVER blocked — the
...
def losses_in_window(trades: list, now_ms: int, window_h: float = WINDOW_H) -> tuple:
    """(losing-trade count, net P&L) over the trailing window.
    trades: [{"pnl": float, "time": ms}, ...] — closed_pnl_history shape."""
