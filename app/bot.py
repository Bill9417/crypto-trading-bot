    ...
    with app.app_context():
        try:
            ...
            for signal in active_signals:
                # Trailing strategies (S3/S4) are managed by a ratcheting stop on
                # closed 1h candles, not the TP1/breakeven/TP2 bracket below.
                if getattr(signal, "manage", "bracket") == "trailing":
                    try:
                        _manage_trailing_signal(signal)
                    except Exception as exc:  # noqa: BLE001 — never break the loop
                        print(f"[trailing] manage error for {signal.symbol}: {exc}")
                    continue
...
def resolve_live_strategy():
    """Validate LIVE_STRATEGY against the registry. Returns (key, manage). Falls
    back to 'default' (S1) with a loud log if the key is unknown, so the live bot
    never trades a typo'd/invalid strategy."""
    import backtest as BT
    key = LIVE_STRATEGY
    if key not in BT.STRATEGIES:
        print(f"[strategy] ⚠ LIVE_STRATEGY={key!r} unknown "
              f"({', '.join(BT.STRATEGIES)}); falling back to 'default' (S1).")
        key = "default"
    manage = "trailing" if BT.STRATEGIES[key].get("sim") is BT.simulate_trade_trailing else "bracket"
    return key, manage


def _alt_ui_row(symbol, timeframe, scan_price, is_tradeable, *, direction="NONE",
                eff=0, status="rejected", reason="", entry=None, sl=None, tp=None,
                tp2=None, strat_name=""):
    """A dashboard signal card for the non-default live path (same keys the S1 path
    emits, so index.html renders it identically)."""
    base_symbol = symbol.split("/")[0]
    return {
        "symbol": symbol, "timeframe": timeframe, "current_price": scan_price,
        "rsi": 0.0, "score": int(eff), "direction": direction,
        "conviction": get_conviction_level(int(eff)) if eff else "—",
        "lights": [], "effective_lights": int(eff), "trend_aligned": True,
        "smc_blocked": False, "tradeable": is_tradeable,
        "trade_status": status, "trade_queued": status == "queued",
        "trade_recorded": status in {"active", "pending", "tp", "sl"},
        "record_status_reason": reason,
        "entry": format_price(entry) if entry else None,
        "sl": format_price(sl) if sl else None,
        "tp": format_price(tp) if tp else None,
        "tp2": format_price(tp2) if tp2 else None,
        "details": [strat_name] if strat_name else [],
        "smc": {"zone_type": "neutral", "zone_label": "", "summary": "", "details": [],
                "trend": "", "swing_high": None, "swing_low": None,
                "dist_to_high_pct": None, "dist_to_low_pct": None},
        "tv_url": f"https://www.tradingview.com/chart/?symbol=BINANCE:{base_symbol}{QUOTE_ASSET}.P",
    }


def _overlay_existing_state(row, st):
    """Overlay a recent DB record's state onto a UI row (so a filled/closed trade
    shows 'active'/'tp'/'sl' instead of a fresh 'queued'/'rejected')."""
    if not st:
        return
    row["trade_status"] = st["trade_status"]
    row["trade_queued"] = st["trade_status"] == "queued"
    row["trade_recorded"] = st["trade_status"] in {"active", "pending", "tp", "sl"}
    row["record_status_reason"] = st["record_status_reason"]
    for k in ("entry", "tp", "tp2", "sl", "exit_price"):
        if st.get(k) is not None:
            row[k] = st[k]


def scan_symbol_alt_strategy(symbol, timeframe, ohlcv, btc_regime, is_tradeable,
                             recent_record_states, live_position_symbols=frozenset()):
    """Evaluate one symbol with the active non-default live strategy, queue + place
    a resting order if it qualifies, and return a dashboard UI row. Mirrors the
    Strategy-1 queue/capacity/notify flow but uses the registry decision engine and
    tags the trade 'bracket' (S2) or 'trailing' (S3/S4)."""
    global queued_signals
    import backtest as BT
    key, manage = resolve_live_strategy()
    strat = BT.STRATEGIES[key]

    if len(ohlcv) < BT.WINDOW:
        return _alt_ui_row(symbol, timeframe, ohlcv[-1][4] if ohlcv else 0,
                           is_tradeable, reason="thin history", strat_name=strat["name"])
    window = ohlcv[-BT.WINDOW:]
    scan_price = window[-1][4]

    ema50_4h, _sh, _sl, _err = check_4h_trend(symbol, scan_price)
    try:
        res = strat["fn"](window, ema50_4h, btc_regime)
    except Exception as exc:  # noqa: BLE001 — a bad bar must not kill the scan
        print(f"[strategy] {key} eval error for {symbol}: {exc}")
        res = None

    if not res:
        row = _alt_ui_row(symbol, timeframe, scan_price, is_tradeable,
                          reason=f"no {strat['name']} setup", strat_name=strat["name"])
        _overlay_existing_state(row, recent_record_states.get((symbol, "LONG"))
                                or recent_record_states.get((symbol, "SHORT")))
        return row
    ...
    if manage == "trailing":
        ...
        if not atr or atr <= 0:
            atr = abs(entry - sl) / max(ATR_SL_MULTIPLIER, 0.5)
        trail_dist = atr * BT.CHANDELIER_ATR_MULT

    status, reason = "rejected", f"{strat['name']} setup"
    if not is_tradeable:
        reason = f"watch-only — outside top {TOP_SYMBOL_LIMIT} tradeable"
    elif has_active_trade:
        status = "active"
        reason = existing_trade_state.get("record_status_reason") or "trade active"
    elif symbol in live_position_symbols:
        reason = ("skipped — a live position already exists on the exchange "
        ...
        if is_new_queue and not executor.has_capacity():
            reason = (f"concurrency cap full — {executor.concurrent_commitments()}/"
                      f"{MAX_CONCURRENT_POSITIONS} committed; not queued this scan")
        else:
            # Place the resting order FIRST; only track it as queued when an order is
            # actually on the book. A skipped/rejected order returns None and must NOT
            # become a phantom PENDING when price later touches the entry.
            placed = None
            try:
                placed = executor.place_resting_order(symbol, direction_str, entry, sl, tp1, tp2,
                                                      int(eff), True, manage=manage)
            except Exception as exc:  # noqa: BLE001
                print(f"[executor] place_resting_order error for {symbol}: {exc}")
            if placed:
                queued_signals[queue_key] = {
                    "symbol": symbol, "timeframe": timeframe, "direction": direction_str,
                    "entry": entry, "tp": tp1, "tp2": tp2, "sl": sl,
                    "lights_count": int(eff), "rsi_value": None, "trend_aligned": True,
                    "smc_zone": None, "manage": manage, "trail_dist": trail_dist,
                    "strategy": key,
                }
                status, reason = "queued", "queued from latest scan; waiting for price to reach entry"
                if is_new_queue:
                    card_tp = None if manage == "trailing" else tp2
                    foot = "⏳ waiting for entry · trailing stop" if manage == "trailing" \
                        else "⏳ waiting for entry"
                    notify(
                        format_trade_card(
                            "📥 QUEUED", symbol, direction_str, entry, sl, card_tp,
                            timeframe=timeframe, lights=int(eff), footer=foot,
                        ),
                        parse_mode="HTML",
                    )
            else:
                queued_signals.pop(queue_key, None)
                reason = "entry order not placed (below min order size or exchange-rejected) — not queued"

    row = _alt_ui_row(symbol, timeframe, scan_price, is_tradeable, direction=direction_str,
                      eff=int(eff), status=status, reason=reason, entry=entry, sl=sl,
                      tp=tp1, tp2=tp2, strat_name=strat["name"])
    _overlay_existing_state(row, existing_trade_state)
    return row


def _manage_trailing_signal(signal):
    """Live Chandelier trailing-stop management for an open S3/S4 position. Ratchets
    the exchange stop on each newly-closed 1h candle (mirrors simulate_trade_trailing)
    and closes the trade when the stop is pierced or the hold cap is reached. Mutates
    the ORM `signal`; the caller's app_context commits it."""
    import backtest as BT
    symbol, direction = signal.symbol, signal.direction
    is_long = direction == "LONG"
    entry = signal.entry_price
    trail = signal.trail_dist
    if not trail or trail <= 0:   # fallback from the initial structure stop
        trail = abs(entry - signal.sl_price) * BT.CHANDELIER_ATR_MULT / max(ATR_SL_MULTIPLIER, 0.5)
    stop = signal.cur_stop if signal.cur_stop is not None else signal.sl_price
    peak = signal.peak if signal.peak is not None else entry
    trough = signal.trough if signal.trough is not None else entry
    last_ts = signal.last_bar_ts or 0

    try:
        oh, _ = get_symbol_ohlcv(symbol, "1h", limit=BT.MAX_HOLD_BARS_TRAIL + 5)
    except Exception as exc:  # noqa: BLE001
        print(f"[trailing] ohlcv fetch failed for {symbol}: {exc}")
        return
    tf_ms = 3600 * 1000
    now_ms = int(time.time() * 1000)
    closed = [c for c in oh if int(c[0]) + tf_ms <= now_ms]   # closed bars only
    ...
    for c in closed:
        ...
        hi, lo = c[2], c[3]
        if is_long:
            if lo <= stop:                               # stop hit (checked before raising)
                exit_price, exited, exit_reason, last_ts = stop, True, "trailing stop", ts
                break
            peak = max(peak, hi)
            if peak - trail > stop:                      # ratchet up only
                stop, moved = peak - trail, True
        else:
            if hi >= stop:
                exit_price, exited, exit_reason, last_ts = stop, True, "trailing stop", ts
                break
            trough = min(trough, lo)
            if trough + trail < stop:                    # ratchet down only
                stop, moved = trough + trail, True
        last_ts = ts

    if (not exited) and signal.timestamp:
        held_h = (get_now_taiwan().replace(tzinfo=None) - signal.timestamp).total_seconds() / 3600.0
        if held_h >= BT.MAX_HOLD_BARS_TRAIL:
            exit_price = closed[-1][4] if closed else entry
            exited, exit_reason = True, "max hold reached"

    if exited:
        pnl = ((exit_price - entry) / entry * 100) if is_long else ((entry - exit_price) / entry * 100)
        signal.status = 'TP' if pnl > 0 else 'SL'
        signal.exit_price = exit_price
        signal.exit_timestamp = get_now_taiwan().replace(tzinfo=None)
        signal.pnl_pct = pnl
        signal.cur_stop, signal.peak, signal.trough, signal.last_bar_ts = stop, peak, trough, last_ts
        if pnl < 0:
            record_daily_loss(pnl)
        try:
            # Force the exchange position FLAT. If the live trailing stop already
            # fired (bot kept up) this is a safe no-op ("no open position"); if the
            # stop was stale because the bot was down, this market-closes it now so
            # the DB and the exchange never diverge into a naked position.
            if executor.is_live():
                executor.close_position_market(symbol)
            else:
                executor.on_trade_closed(symbol, direction)
        except Exception as exc:  # noqa: BLE001
            print(f"[trailing] close error for {symbol}: {exc}")
        update_signal_state_in_ui(symbol, direction,
            trade_status=('tp' if pnl > 0 else 'sl'),
            reason=f"{exit_reason} — {'WIN' if pnl > 0 else 'LOSS'} {pnl:+.1f}%",
            entry=entry, sl=stop, exit_price=exit_price)
        notify(f"{'🏆' if pnl > 0 else '🛑'} TRAILING EXIT  {dir_tag(direction)} · {pair_name(symbol)}  "
               f"{pnl:+.1f}%\n{exit_reason}")
        return

    # still open — persist the ratchet and move the live stop if it advanced.
    signal.peak, signal.trough, signal.last_bar_ts = peak, trough, last_ts
    if moved:
        signal.cur_stop = stop
        try:
            executor.move_stop(symbol, direction, stop, reason="trail")
        except Exception as exc:  # noqa: BLE001
            print(f"[trailing] move_stop error for {symbol}: {exc}")
        update_signal_state_in_ui(symbol, direction, trade_status="active",
            reason=f"trailing stop @ {format_price(stop)}", entry=entry, sl=stop)
        ...
        for symbol in symbols:
            ...
            for timeframe in TIMEFRAMES:
                try:
                    ...
                    # ── Non-default live strategy (S2/S3/S4): handled by a
                    #    self-contained registry path; the Strategy-1 block below is
                    #    skipped entirely. With LIVE_STRATEGY="default" (the safe
                    #    default) this branch never runs and S1 behaves exactly as
                    #    before. ──
                    if LIVE_STRATEGY != "default":
                        alt_row = scan_symbol_alt_strategy(
                            symbol, timeframe, ohlcv, btc_regime,
                            symbol in tradeable_symbols, recent_record_states,
                            live_position_symbols,
                        )
                        existing_queued.pop((symbol, timeframe), None)
                        if alt_row:
                            all_signals_for_ui.append(alt_row)
                        continue
