...
| `Sykes_Vegas_Tunnel_Pro.pine` | Cut down from a user-supplied "Sykes Strategy & Market Structure Combined" script: dropped the HH/LL market-structure mode, the 4 free-text timezone slots, and QQE MOD. Trend is ONLY the two Vegas tunnels (EMA144/169 small, EMA576/676 large) now. Entries require the existing retest→breakout mechanic PLUS a new MACD momentum gate (fast>slow AND histogram expanding) AND a volume-surge gate (≥1.2× its average, tunable), plus an optional "one signal at a time" lock — fewer, stricter BUY/SELL prints by design. Fixed 5m/15m/30m trend panel replaces the old configurable monitors (optional 15m+30m alignment gate available, off by default). `overlay=false` + `force_overlay=true` puts the tunnels/signals/panel on the price chart while MACD + relative volume render in a genuinely separate pane below. **v2**: the first cut modulated the MACD histogram's *opacity* by volume, which read as barely-there. Volume is now its own visible bar series, converted into the MACD histogram's own rolling standard-deviation units — an asset-agnostic way to keep two differently-scaled series (volume counts vs. MACD's price units) legible on one shared axis without a fixed, per-symbol guess. ⚠ Never opened in TradingView — passed the repo's forward-reference linter (`check_pine.py`) and a manual paren/indent check, but needs a visual pass on a real chart before trusting the signals. |
| `All-in-One_ULTIMATE_Pro.pine` | Forked from `All-in-One_ULTIMATE.pine`. **Volume Profile removed** (⑫ was fully self-contained — nothing outside it read a `vp`-prefixed identifier — deleted wholesale; the Confidence Meter's Volume factor never depended on it, it always used its own rolling per-bar volume-weighted mean). **Confidence Panel (⑪) + MTF Bias Panel (⑬) redesigned**: each row's *background*, not just its text, tints by that row's own bull/bear reading — reserved for rows with a direction, so a tint always means "this row has an opinion"; the factor breakdown sits behind its own labeled divider; the MTF panel's Support/Resistance column is now two separate columns (Pine can't color part of one cell's text) — Resistance in the indicator's red, Support in its teal. **Double Tunnel fixed**: Tunnel 2 used to be hidden by Clean Chart Mode (which defaults ON), so "Double Tunnel" only ever showed one line out of the box — now always-on core drawing. Its default periods also changed from Gilbert0967's generic 2× convention (288/338) to Sykes' own 4× big-tunnel periods (576/676), matching `Sykes_Vegas_Tunnel_Pro.pine`'s EMA576/676 — this is the one change here that alters the Confidence Meter's Tunnel factor and the MTF scorer versus the source file, deliberately. **⑭ MACD + Volume pane added** — the source file had neither. Getting a real separate pane (not squares crammed onto the price scale) meant flipping the whole script from `overlay=true` to `overlay=false` and mechanically adding `force_overlay=true` to all ~57 pre-existing plot/fill/hline/bgcolor/label/line/box/table calls so nothing shifted panes; verified by grep count (every old call flagged, only the 6 new MACD/Volume calls aren't) rather than by eye. Same asset-agnostic volume normalization as `Sykes_Vegas_Tunnel_Pro.pine`'s v2 fix. Everything else — EMA stack, Trend Lines, MSB-OB, SMC, Reactive S/R Zones, the Confidence Meter's own math — is untouched. ⚠ Never opened in TradingView — passed `check_pine.py` and manual paren/indent/tab checks, but needs a visual pass before trusting it. |
